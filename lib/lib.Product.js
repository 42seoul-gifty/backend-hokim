...
const makeSqlCondition = (gender, age, price) => {
  const target = [gender, age, price];
  const columnName = ["gender", "age", "price"];
  const condition = ["", "", ""];
  var result = "";
  for (let i = 0; i < target.length; i++) {
    target[i]?.forEach((elem) => {
      if (condition[i] == "") condition[i] += " ( ";
      if (condition[i].includes("Preference")) condition[i] += " or ";
      condition[i] += `Preference.${columnName[i]}_id = ${elem}`;
    });
    if (condition[i] != "") {
      condition[i] += ")";
...
const findFilteredProduct = async (gender, age, price) => {
  try {
    const products = await sequelize.query(
      `
            SELECT DISTINCT Product.id, category, name, description, detail, thumbnail, price, brand
                  from Product
...
const findAdminFilteredProduct = async (gender, age, price) => {
  try {
    const condition = makeSqlCondition(gender, age, price);
    const products = await sequelize.query(
      `
          WITH FilterdProduct AS(
            SELECT DISTINCT Product.id, name, category, description, detail, thumbnail, price, brand
                  from Product
                  left join ProductPreference on Product.id = ProductPreference.product_id
                  left join Preference on Preference.id = ProductPreference.preference_id
                   ${condition}
